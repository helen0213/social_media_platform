...
.container-md {
    ...
    <div class="container-md center">
        <h2>Login</h2>
            Username: <input type="text" name="currentUsername"> <br /><br />
            Password: <input type="text" name="password"> <br /><br />
            <input type="hidden" id="userLoginRequest" name="userLoginRequest">
            <input type="submit" value="Login as User" name="UserLoginSubmit"></p> <br /><br />
            <a class="nav-link" href="user-signup.php">New user? Sign up here</a>
    </div>
    </form>
    ...
    function handleUserLoginRequest() {
        ...
        $result = executePlainSQL("SELECT password from Users where username = '{$enteredUsername}' AND password = '{$enteredPassword}'");

        while ($row = OCI_Fetch_Array($result, OCI_BOTH)) {
            if (!isset($row[0])) {
                echo "Username or password is incorrect.";
            } else {
                file_put_contents("userConfig.php", "<?php return " . $enteredUsername . " ?>");
    ...
    function handleDisplayRequest() {
        ...
        printResult($result2, "USERNAME", "TITLE");
...
// A better coding practice is to have one method that reroutes your requests accordingly. It will make it easier to add/remove functionality.
    function handleGETRequest() {
        if (connectToDB()) {
            if (array_key_exists('userLoginRequest', $_POST)) {
                if(!isset($_POST['currentUsername']) || !isset($_POST['password'])) {
